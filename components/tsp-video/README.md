...
index文件夹下
tabVideo（tab视频页 顶部导航栏） -> tabs-page（视频和商城页切换） -> appIndex( 添加短视频组件，双版本兼容 )

static/js/vodData.js 假数据

startData()/* 模拟请求 */

视频组件参数
tabIndex 底部导航栏index
tabItem（顶部导航栏id+name）
pageList tabItem集合
swId 当前页面tabItem.id
pageIndex 当前页面tabItem.index	vodList 当前播放的视频数组
vodIndex 当前视频下标
视频组件方法
refreshSquare(res)
initVod(res,0); //0是播放的下标（默认播放下标是0） 
